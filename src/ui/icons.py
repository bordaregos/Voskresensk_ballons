...
_PATHS = {
    ...
    # Контур папки -- в отличие от "files" (два перекрывающихся листа,
    # визуально читается как "копировать"), используется там, где нужно
    # однозначно отличить папку от документа -- см. дерево objectsTree
    # (_refresh_objects_tree()) и docs/design/вводная_часть.html.
    ...
    # Зеркало "upload" (тот же лоток снизу, стрелка вниз вместо вверх) --
    # «Сохранить как» на карточке загруженного шаблона варианта (см.
    # _save_variant_template_as(), src/ui/main_window.py).
    "download": (
        '<path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-2"/>'
