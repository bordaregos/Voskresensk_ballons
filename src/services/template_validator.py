...
# ``{{r field }}`` -- докстпл-тег "сырой XML" (для RichText, переносы строк
# внутри значения -- см. MainWindow.RICH_TEXT_FIELDS) -- префикс "r" (с
# обязательным пробелом после, чтобы не съесть начало реальных имён полей
# вроде "reg_number") распознаётся и отбрасывается так же, как фильтр выше.
_SCALAR_TAG_RE = re.compile(r"\{\{\s*(?:r\s+)?([A-Za-z_][\w.]*)(?:\s*\|[^}]*)?\s*\}\}")
