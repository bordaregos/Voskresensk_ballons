...
# Второй, независимый слот конструктора наравне с титульным листом --
# «Вводная часть» (нумерованные пункты вроде «1.1. На основании требований
# п.198 ФНП ТТ ...»). Тот же паттерн, что и TITLE_VARIANTS выше: встроенных
# вариантов нет, реестр пуст, каждый вариант заводится через UI (кнопка
# «Добавить» в группе «Вводная часть» сайдбара конструктора). Держится
# отдельным словарём (не переиспользует TITLE_VARIANTS) -- варианты
# титульного листа и вводной части выбираются и комбинируются в документе
# независимо друг от друга (см. src/ui/main_window.py, второй слот
# конструктора).
INTRO_VARIANTS: Dict[str, TitleConfig] = {}

# В отличие от DEFAULT_TITLE_SUBTITLE_FIELDS -- пустой список, а не
# doc_number/reg_number/doc_date/object_title: те поля осмысленны для
# титульного листа (реквизиты документа в целом), но не для вводной части,
# где каждый пункт -- собственное произвольное поле, заводимое вручную через
# «+ Новое поле» (см. мокап docs/design/вводная_часть.html, где по той же
# причине новый вариант вводной части тоже стартовал бы с пустого набора,
# если бы не общая заготовка). Каталог подписей (TITLE_FIELD_LABELS) общий
