...
`src/services/table_formulas.py` (чистые функции, тесты
`tests/test_table_formulas.py`) + строка «Формула» в `table_editor_dialog.py`.
Ячейка-формула — единственный токен `{"type": "formula", "expr", "decimals"}`.
Синтаксис как в русском Excel: ссылки `B2`/`$B$2`, диапазоны `A1:C3`,
`{id_поля}`, аргументы через `;`, функции СУММ/СРЗНАЧ/МИН/МАКС/СЧЁТ/ABS/
ОКРУГЛ/КОРЕНЬ. «Заполнить вниз/вправо» протягивает формулу со сдвигом
относительных ссылок; удаление строки/столбца правит ссылки (битые —
`#ССЫЛКА!`). В `.docx` `_build_table_element()` считает всю таблицу через
`evaluate_table()` (каждая ячейка резолвится один раз — рандом-чип даёт одно
значение и ячейке, и ссылающимся на неё формулам); ошибка печатается как
`#ОШИБКА`. В редакторе предпросмотр использует пробное значение рандом-полей.

### Чипы-результаты таблицы

Секция «Чипы-результаты» в `table_editor_dialog.py`: «+ Чип-результат»
заводит поле в каталоге и запись `outputs` таблицы —
`[{"field_id", "expr", "decimals"}]` (`expr` — формула как в ячейке, по
умолчанию адрес активной ячейки). Считаются одним проходом с таблицей
(`table_formulas.evaluate_table_outputs()`). После «Сохранить» чип
появляется в реквизитах под чипом таблицы (`_add_table_output_rows()`) с
read-only значением; в документ значение уходит из `get_form_data()`, тексты
таблицы кэшируются в `_table_eval_cache` для `_build_table_element()`, чтобы
рандом-чип не дал в таблице и в чипе разные значения. Чип не входит в
`subtitle_fields` варианта; при отмене/удалении чипа или таблицы запись
чистится из каталога (`_purge_catalog_fields()`).
